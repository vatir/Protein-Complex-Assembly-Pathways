...
class ODEFunc(object):
    ...
    def GetRDataFromFiles(self, filename):
        try:
            ...
            with open(filename,"r") as ReactFile:
                ...
                ReactList = list(map(lambda x: x.split("\t"), ReactList))
                for R in ReactList:
                ...
                SortList = list(map(list,list(np.array(self.Reactions)[:,(3,0,1)])))
                L = range(len(SortList))
                for i in L:
                    SortList[i].append(i)
                SortOrder = np.array(sorted(SortList))[:,3]
                self.Reactions = list(map(list,list(np.array(self.Reactions)[SortOrder])))
    ...
    def GenCoef(
        ...
        """
#        if not(A0==0.0) or not(B0==0.0):
        self.SetInitialConc(A0, B0)
    ...
    def SetInitialConc(self, A0=0.0, B0=0.0):
