...
class MPLGPU(object):
    def __init__(self, Width = "double", Aspect = 4.0/3.0, axes_aspect=4.0/3.0, subplot=111, PrimaryFig = False, *args, **kwargs):
...
#                top=None,
            ...
            self._Plot = self._MainFig.add_subplot(subplot)#, aspect = axes_aspect)
            
        self.SetAxesProperties(self._Plot)

    def SetAxesProperties(self, axes):
        axes.tick_params(
    ...
    def SetAxisLabel(self, string, axis):
        if axis == "x":
            self._Plot.set_xlabel(string)
        if axis == "y":
            self._Plot.set_ylabel(string)

    def Title(self, string):
        self._Plot.set_title(string, y = 1.04)

    def XLabel(self, string):
        self.SetAxisLabel(string, "x")

    def YLabel(self, string):
        self.SetAxisLabel(string, "y")

    def SetXRange(self, minimum, maximum):
        self._Plot.set_xlim([minimum, maximum])

    def SetYRange(self, minimum, maximum):
        self._Plot.set_ylim([minimum, maximum])
...
class LinePlot(MPLGPU):
    def __init__(self, YAxisInPercent = True, *args, **kwargs):
...
class RankWBinaryImages(MPLGPU):
    def __init__(self, *args, **kwargs):
        super(RankWBinaryImages, self).__init__(*args, **kwargs)
        self._Plot        = self._MainFig.add_subplot(111)
        self.SetAxesProperties(self._Plot)
...
class ContourPlot(MPLGPU):
    def __init__(self, *args, **kwargs):
        super(ContourPlot, self).__init__(axes_aspect = "equal", *args, **kwargs)
        self.SetLogLog()
