...
class ODEFunc(object):
	...
	def GenCoef(
		...
		for R in self.Reactions:
			...
			Keff = Kp*(Kd1**R[4])*(Kd2**R[5])*np.exp((R[4]+R[5]-1)*BondCoef) # Bond Count in Delta G Pos (V2)
			# Keff = Kp*(Kd1**R[4])*(Kd2**R[5])*np.exp(BondCoef) # Bond Count removed from Delta G Pos (NoBondsInPos)
			#Keff = Kp*(Kd1**R[4])*(Kd2**R[5]) # No Pos (NoPos)
			#Keff = 0.0 # (NoDis)
			
			
			# Keff = (Kp**(R[4]+R[5]))*(Kd1**R[4])*(Kd2**R[5])*np.exp((R[4]+R[5]-1)*BondCoef) # Version from 8/12/2017
...
#		print "This Function Should NEVER be used!!!!"
